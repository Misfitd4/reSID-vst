...
EnvelopeGenerator::EnvelopeGenerator()
{
  ...
  set_envelope_quirks_enabled(true);
...
void EnvelopeGenerator::reset()
{
  ...
  clean_zero_adsr_muted = false;
  clean_sustain_level = sustain_level[sustain];
...
void EnvelopeGenerator::set_envelope_quirks_enabled(bool enabled)
{
  envelope_quirks_enabled = enabled;
  if (!enabled) {
    envelope_pipeline = 0;
    exponential_pipeline = 0;
    state_pipeline = 0;
...
void EnvelopeGenerator::set_clean_sustain_level(reg8 level)
{
  clean_sustain_level = level;
}
...
void EnvelopeGenerator::writeCONTROL_REG(reg8 control)
{
  ...
  if (gate != gate_next) {
    if (!envelope_quirks_enabled) {
      clean_zero_adsr_muted =
        gate_next && attack == 0 && decay == 0 && clean_sustain_level == 0 && release == 0;

      if (clean_zero_adsr_muted) {
        envelope_counter = 0;
        env3 = 0;
        state = RELEASE;
        next_state = RELEASE;
      ...
      state = gate_next ? ATTACK : RELEASE;
      next_state = state;
      state_pipeline = 0;
      envelope_pipeline = 0;
      exponential_pipeline = 0;
      ...
      hold_zero = false;
      rate_counter = 0;
      rate_period = rate_counter_period[gate_next ? attack : release];
      gate = gate_next;
      return;
    }
...
void EnvelopeGenerator::writeATTACK_DECAY(reg8 attack_decay)
{
  ...
  if (clean_zero_adsr_muted && (attack != 0 || decay != 0 || sustain != 0 || release != 0)) {
    clean_zero_adsr_muted = false;
  }
  if (state == ATTACK) {
  ...
  if (!envelope_quirks_enabled && rate_counter >= rate_period) {
    rate_counter = 0;
  }
}
...
void EnvelopeGenerator::writeSUSTAIN_RELEASE(reg8 sustain_release)
{
  ...
  else if (!envelope_quirks_enabled && state == DECAY_SUSTAIN) {
    rate_period = rate_counter_period[decay];
  }
  if (!envelope_quirks_enabled && rate_counter >= rate_period) {
    rate_counter = 0;
