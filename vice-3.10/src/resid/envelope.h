...
public:
  ...
  void set_envelope_quirks_enabled(bool enabled);
  void set_clean_sustain_level(reg8 level);
...
protected:
  ...
  reg8 gate;
  bool envelope_quirks_enabled;
  bool clean_zero_adsr_muted;
  reg8 clean_sustain_level;
...
void EnvelopeGenerator::clock()
{
  ...
  if (unlikely(!envelope_quirks_enabled)) {
    if (unlikely(clean_zero_adsr_muted)) {
      envelope_counter = 0;
      env3 = 0;
      return;
    }
    ...
    if (state == ATTACK || ++exponential_counter >= exponential_counter_period) {
      ...
      const reg8 sustain_target = clean_sustain_level;
      ...
      case ATTACK:
        ...
        break;
      case DECAY_SUSTAIN:
        if (envelope_counter > sustain_target) {
          --envelope_counter;
        } else if (envelope_counter < sustain_target) {
          ++envelope_counter;
        }
        break;
      case RELEASE:
...
void EnvelopeGenerator::clock(cycle_count delta_t)
{
  ...
  if (unlikely(!envelope_quirks_enabled && rate_counter >= rate_period)) {
    rate_counter = 0;
  }

  if (unlikely(!envelope_quirks_enabled && clean_zero_adsr_muted)) {
    envelope_counter = 0;
    env3 = 0;
    rate_counter = 0;
    return;
  ...
  while (delta_t) {
    ...
    if (state == ATTACK || ++exponential_counter == exponential_counter_period) {
      ...
      const reg8 sustain_target = envelope_quirks_enabled ? sustain_level[sustain] : clean_sustain_level;
      ...
      case ATTACK:
        ...
        // then to attack. This has been verified by sampling ENV3.
        //
        if (envelope_quirks_enabled || envelope_counter < 0xff) {
          ++envelope_counter &= 0xff;
        }
        if (unlikely(envelope_counter == 0xff)) {
          state = DECAY_SUSTAIN;
        ...
        break;
      case DECAY_SUSTAIN:
        if (envelope_quirks_enabled) {
          if (likely(envelope_counter != sustain_target)) {
            --envelope_counter;
          }
        } else if (envelope_counter > sustain_target) {
          --envelope_counter;
      ...
      case RELEASE:
        ...
        //
        if (envelope_quirks_enabled || envelope_counter > 0x00) {
          --envelope_counter &= 0xff;
        }
        break;
      case FREEZED:
