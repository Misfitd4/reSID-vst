...
void SID::set_envelope_quirks_enabled(bool enabled)
{
  for (int i = 0; i < 3; i++) {
    voice[i].envelope.set_envelope_quirks_enabled(enabled);
  }
}

void SID::set_voice_clean_sustain_level(unsigned int voice_index, reg8 level)
{
  if (voice_index >= 3) {
    return;
  }
  voice[voice_index].envelope.set_clean_sustain_level(level);
}
