...
public:
  ...
  void set_voice_clean_sustain_level(unsigned int voice, reg8 level);
  ...
  // Unmixed oscillator/envelope output for scope visualization.
  int voice_output(unsigned int index)
  {
    return index < 3 ? voice[index].output() : 0;
  }
